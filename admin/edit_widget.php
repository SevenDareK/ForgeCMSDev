                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <i class="fa fa-cog"></i> Modifier un Widget
                            </li>
                        </ol>
                    </div>
                </div>
...
if (@$_GET['voir']) {
    $voir = $pdo->prepare("SELECT * FROM widget WHERE id=:id");
    $voir->execute(array(
        'id' => $_GET['voir']
        ));
    $donnees = $voir->fetch();
    ...
    $maj = $pdo->prepare("UPDATE widget SET title=:title, content=:content WHERE id=:id");
    $maj->execute(array(
        'title' => $title,
        'content' => $content,
        'id' => $_GET['voir']
        ));
    echo "<script type='text/javascript'>document.location.replace('widget?succes=1');</script>";
    }
?>
...
<link rel="stylesheet" type="text/css" href="editor/bootstrap3-wysihtml5.css">
    ...
    echo "<script type='text/javascript'>document.location.replace('widget');</script>";
}
} else {
    echo "<script type='text/javascript'>document.location.replace('widget');</script>";
}
